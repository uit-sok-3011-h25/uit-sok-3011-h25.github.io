...
Derivating matrices follows similar principles to derivating polynomials [@Cayley1858]. For instance:
...
where the *excess* return of the front is $\mu_{front}={\mu}^{\prime}\mathbf{a}$ 
...
Where the last equality follows from Equation @eq-opt_port_B. The normalized optimal portfolio is then
...
The expected excess return of the optimal portfolio on the frontier, is then 
...
We can obtain the y-coordinate, the volatility of the optimal portfolio, by inserting its excess return into the portfolio front expression @eq-opt_port_front. After som simplification, turns out:
$$
...
Let us now draw a tangency line from the risk free interest rate `rf` to the optimal portfolio point. The slope will be the Sharp-ratio, the ratio of excess return to volatility [@Sharpe1964]. For the optimal portfolio, it is

$$
S=\frac{\mu_{opt} }{ {\sigma}_{opt} }=\sqrt{C}
$${#eq-opt_port_sol_sharpe}

Where the last equatlity follows from the previous definitions.


- **Challenge**: Derive $\mu_{opt}$, ${\sigma}^2_{opt}$ and S mathematically. 
...
where $B$ is the sum of the optimal portfolio, as previously defined in @eq-opt_port_B
