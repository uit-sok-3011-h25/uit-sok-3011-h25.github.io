...
To make this a bit more tangible, let us assume $f(x)$ represents the standard normal distribution (variance = 1 and mean = 0), and assume we categorize the outcomes $x$ into five intervals. In Python, we can obtain discrete equivalents of the normal probabilities [@Gauss1809] of a set of intervals in the range `-maxmin` to `maxmin`, each with a width of `dx`, as follows:
...
import utility
fig, ax = utility.plot_descrete_and_normal(p, x, dx, maxmin, plt)
...
* **Challenge 3**: Now, assume the utility function is $f(x) = -\mathrm{log}(\rho x)$ (the natural logarithm). What is the expected utility for different values of $x$?
...
Let us now try to compare this with our approximated expected utility above. If we have a standard normal distribution, so $\sigma=1$ and $\mu=0$. The risk aversion coefficient is $\rho=5$. The exact and approximate expected utility are
...
* **Challenge 4**: Calculate the approximate expected utility with some other parameters and a non-standard normal distribution ($\sigma<>1$ and $\mu<>1$), and see if the approximate is still a good approximation.
...
* **Challenge 6**: Use data from [https://titlon.uit.no](https://titlon.uit.no) to create a histogram, and overlay the normal distribution to see if the empirical distribution has fat tails.
* **Challenge 7**: Try to fit a mixed normal distribution to the empirical distribution.
...
We can then calculate the return series and create a histogram. The period mean and standard deviation are also calculated.
...
The normal distribution and the mixed normal are now calculated, based on the estimated mean and standard deviation, and some assumptions on how the mixed normal distributions deviate from the single normal. In this example, it is just done by trial and error, which leads to a weight of 80% on a normal distribution with 40% of the estimated volatility, and 20% on a normal distribution with 200% of the estimated volatility.

This is then added to the existing chart:
