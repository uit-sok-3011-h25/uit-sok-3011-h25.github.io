...
- **Challenge 2**: See if a finer categorization will give a more exact variance estimate.
...
- **Challenge 4**: Calculate the approximate expected utility with some other parameters and a non standard normal distribution ($\sigma<>1$ and $\mu<>1$, and see if the approxmate is still a good approximation.
...
- **Challenge 5**: Show that when multiplying a random variable with a constant, the variance is the variance of the random variable times the square of the constant.
