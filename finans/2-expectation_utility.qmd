...
To make this a bit more tangible, let us assume f(x) represents the standard normal distribution (variance=1 and mean=0), and assume we collect the outcomes x into five intervals. According to the normal distribution [@Gauss1809], each outcome will then have the probabilities shown in the following figure (the code to is to long to have in this document, but it can be found in the local module `utility.py` in this repository):
...
dx = 2
maxmin = 4
fig, ax, p, x = utility.normaldist(dx, maxmin)

print(f"Probabilities: {np.round(dx*p,3)}")
print(f"Outcomes: {np.round(x,2)}")
```

The argument `dx` is the widht of the bars in the figure, and `maxmin` determines which range of values should be plottet. `maxmin = 4` means that values from -4 to 4 are plotted. 

The probability of each outcome (bar) and the associated outcmes, are returned as p and x, respectively. 
...
- **Challenge 2**: Now, assume the utility function is $f(x) = -\mathrm{e}(-\rho x)$ (the natural logarithm). What is the expected utility for different values of $dx$?
...
Consider an individual with an exponential [@Euler1748] utility function:
...
Below is the utility function defiend as the function `ufunc(x, rho)`, and both the expected utitlity and the utility of the expected value are calculated, assuming $\rho = 1.0$:
...
expected_utility = np.sum(p_gamble * 
                                u_func(x_gamble, RHO))
expected_value = np.sum(p_gamble * 
                                x_gamble)
utility_exp_value = u_func(expected_value, RHO)

print(f"Expected utility: {expected_utility}")
print(f"Expected value: {expected_value}")
print(f"Utility of expected value: {utility_exp_value}")
...
import utility
fig,ax = utility.plot(RHO, x_gamble, p_gamble)
...
minmax = 4
RHO = 5
fig, ax, p, x = utility.normaldist(dx, minmax) 
expected_utility = np.sum(p*dx*utility.u_func(x, RHO))


print(f'Exact expected utility:{-np.exp(-RHO*(0.0-0.5*RHO*1.0))}')
print(f'Approximate expected utility:{expected_utility}')
```

Given these descrete probabilties and outcomes, we can calculate the expected utility  of  $u(x) = - \mathrm{e}(-\rho x)$ by multiplying probabilites by outcomes in terms of utility: 
...
Let us now try to compare this with our apporxmated expeted utility above. If we have a standard normal distribution, so $\sigma=1$ and $\mu=0$. The risk aversion coefficient is  $\rho=5$. The exact and approximate expected utility is
